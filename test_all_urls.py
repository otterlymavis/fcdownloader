...
URLS = {
    ...
    "Weibo":         ("https://m.weibo.cn/detail/4904263725515320", "LOGIN-GATED: Weibo auth required from datacenter IPs"),
    "Xiaohongshu":   ("https://www.xiaohongshu.com/explore/65b2e03d000000000103117d", "server /extract is gated -> FAIL here is expected; the APP extracts on-device (use --device)"),
    ...
    "TVer":          ("https://tver.jp/episodes/epc1hdugbk", "GEO/CURRENT-EPISODE: TVer episodes expire after ~1 week; update to a recently aired episode ID"),
    ...
    "TwitCasting":   ("https://twitcasting.tv/ivetesangalo/movie/2357609", "PAST-BROADCAST/AUTH: past TwitCasting recordings require different API access or authentication"),
    "FC2 Video":     ("https://video.fc2.com/en/content/20231103kUan1KHs", "yt-dlp returns a playable HLS URL without a formats list"),
    ...
    "Naver TV":      ("https://tv.naver.com/v/81652", "AUTH/SERVER IP: Naver TV stream extraction requires current Naver session cookies"),
    "Kakao TV":      ("https://tv.kakao.com/channel/2856/cliplink/463538508", "AUTH/SERVER IP: Kakao TV requires auth from datacenter IPs; browser/session path works"),
    ...
    "Oricon":        ("https://www.oricon.co.jp/news/2285123/full/", "BROWSER-ONLY: local Python TLS chain currently fails for oricon.co.jp; browser/session path still works"),
    ...
    "Livedoor Blog": ("https://blog.livedoor.jp/dqnplus/archives/2051261.html", ""),
    "Yahoo Japan articles": ("https://news.yahoo.co.jp/articles/20ed9737a7a5411fbd2456f7df836fca68579d2f", "AUTH/SERVER IP: Yahoo Japan blocks datacenter fetches; browser HTML/cookies path is supported"),
    "Pixiv / Fanbox": ("https://www.pixiv.net/artworks/100000000", ""),
    "Bunshun":       ("https://bunshun.jp/articles/photo/89000", "photo gallery — update ID if 404; gallery IDs are sequential"),
    ...
    "HP+ SPUR":      ("https://spur.hpplus.jp/fashion/", "update path if category returns 404"),
    ...
    "Croissant Online": ("https://croissant-online.jp/life/273608/", "STALE/NETWORK: article may have been removed or site is unreachable — update URL if network error persists"),
    "FRaU":          ("https://frau.tokyo/article/", "tag/index pages rarely expose article images directly; update to a specific article URL"),
    ...
    "Mantan Web":    ("https://mantan-web.jp/article/20240501dog00m200001000c.html", "AUTH/SERVER IP: server fetch currently receives 403; browser/session path is used when available"),
    "Crank In":      ("https://www.crank-in.net/news/186258", ""),
    "CinemaToday":   ("https://www.cinematoday.jp/news/N0153809", ""),
    "eiga.com":      ("https://eiga.com/news/20260610/1/", "signed HLS manifests expire within days — update to a fresh article when manifest 404s"),
    ...
    "Asahi":         ("https://www.asahi.com/topics/", "update to a specific article URL with images; /topics/ always has image thumbnails"),
