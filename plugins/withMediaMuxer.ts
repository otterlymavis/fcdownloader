 ...
 *
 * Writes a Swift + ObjC bridge into the main app target that exposes a
 * `MediaMuxerModule.mux(videoPath, audioPath, outputPath)` JS function.
 * Implementation uses AVAssetExportSession with the Passthrough preset for
 * a lossless `-c copy` style merge of the video and audio tracks.
 *
 * Mirrors the Android `MediaMuxerModule.kt`. Both modules export the same JS
 * name (`MediaMuxerModule`) so `src/lib/ffmpegMux.ts` is platform-agnostic.
 *
 * Usage: add './plugins/withMediaMuxer' to plugins[] in app.config.ts
...
function writeMuxerFiles(projectRoot: string): void {
...
const withMediaMuxerPlugin: ConfigPlugin = (config) => {
...
export default createRunOncePlugin(withMediaMuxerPlugin, 'withMediaMuxer', '1.0.0');
