...
function patchBundleScript(shellScript) {
...
const withIosDeviceDebugBundle = (config) =>
  withXcodeProject(config, (projectConfig) => {
    ...
    for (const phase of Object.values(phases)) {
      ...
      if (phase.name !== `"${PHASE_NAME}"` || typeof phase.shellScript !== 'string') {
        continue;
      }
      phase.shellScript = patchBundleScript(phase.shellScript);
      patched = true;
    }
...
module.exports = createRunOncePlugin(withIosDeviceDebugBundle, PLUGIN_NAME, PLUGIN_VERSION);
